...
学号： & 09023402 \\
姓名： & 金俊贤 \\
链接： & \url{https://github.com/kkcjy/} \\
       & \url{InternetCafe-DataBase.git} \\
时间： & \today
\end{tabular}
\end{center}
\end{titlepage}
...
\section{项目构建}
本项目后端采用 Flask 框架实现，使用 Python 编写，app.py 作为系统主程序入口。
程序通过 Flask 路由机制完成页面跳转及业务接口的定义，前端页面存放于 templates 目录，静态资源文件存放于 static 目录，实现了前后端的基本分离。
系统通过 PyMySQL 与 MySQL 数据库建立连接，在程序启动时自动执行数据库初始化操作，创建所需的数据表及存储过程，保证系统能够直接运行。
系统基本实现了先前所描述的所有功能，例如图fig:search，实现对用户个人信息的精准查询，但是由于精力有限，类似于流水查询的功能并未进行实现。
系统基本实现了前述设计中的各项功能，能够支持用户个人信息的准确查询与管理。但受时间和精力限制，诸如消费流水明细查询等扩展功能尚未完成，有待后续进一步完善。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{../img/search.png}
    \caption{用户信息查询}
    \label{fig:search}
\end{figure}

用户与管理员的各项功能均通过调用数据库存储过程完成，减少了应用层对 SQL 语句的直接操作，提高了系统的稳定性与数据一致性。
系统基本实现了先前所描述的各项功能，如图 \ref{fig:search} 所示，系统能够对用户个人信息进行准确查询。
其余功能已在项目中完整实现，可通过 \url{{https://github.com/kkcjy/InternetCafe-DataBase.git}} 拉取源码进行查看，报告中不再逐一列出。
但受时间和精力限制，诸如消费流水明细查询等扩展功能尚未实现，有待后续进一步完善。
...
同时，系统仍存在一定不足。
其中，对座位状态持续时间的管理较为简单，该类业务逻辑通常由应用层而非数据库层负责实现，因此对系统整体影响有限；
此外，系统尚未针对存取效率及大规模数据处理能力进行专项优化，但在当前应用规模下尚不构成主要问题。
相比之下，更为重要的是数据库备份与恢复机制仍需进一步完善，以切实保障系统数据的安全性与可靠性。
